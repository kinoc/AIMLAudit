...
namespace AIMLbot.Utils
{
    ...
    public class Node
    {
        ...
        public void addCategory(string path, string template, string filename,string totalPath)
        {
            ...
            if (path.Trim().Length == 0)
            {
                ...
                this.categoryPath = totalPath.Trim();
                if (this.categoryPath.Length == 0)
                {
                    Console.WriteLine("Zero path");
                }
                return;
            }
            this.categoryPath = totalPath.Trim();
            ...
            if (this.children.ContainsKey(firstWord))
            {
                ...
                childNode.addCategory(newPath, template, filename,totalPath+ " "+firstWord);
            }
            else
            {
                Node childNode = new Node();
                childNode.word = firstWord;
                childNode.categoryPath =  (totalPath + " " + firstWord).Trim();
        ...
        public string evaluate(string path, SubQuery query, Request request, MatchState matchstate, StringBuilder wildcard)
        {
            ...
            if (request.StartedOn.AddMilliseconds(request.bot.TimeOut) < DateTime.Now)
            {
                ...
                //request.responseTrace += "ANULL" + "\t" + categoryPath + "\n";
                return string.Empty;
            }
            ...
            if (this.children.Count==0)
            {
                ...
                query.UsedFilename = this.filename;
                if (this.template.Length > 0) request.responseTrace += Path.GetFileName(this.filename) + "\t" + this.categoryPath + "\n";
            ...
            {
                query.UsedFilename = this.filename;
                if (this.template.Length > 0) request.responseTrace += Path.GetFileName(this.filename) + "\t" + this.categoryPath + "\t" + this.template + "\n";
                return this.template;
            }
            ...
            // KHC: should be a hashtable check for CHATROOT,AICORE
            if ((this.children.ContainsKey("_")) && (firstWord != "CHATROOT") && (firstWord != "AICORE")&&(!request.skipUnderscores))
            {
                ...
                string result = childNode.evaluate(newPath, query, request, matchstate, newWildcard).Trim();
            ...
            if (this.children.ContainsKey(firstWord))
            {
                ...
                // matchstate
                StringBuilder newWildcard = new StringBuilder();
                string result = childNode.evaluate(newPath, query, request, newMatchstate, newWildcard).Trim();
            ...
            if ((this.word == "_") || (this.word == "*"))
            {
                ...
                return this.evaluate(newPath, query, request, matchstate, wildcard).Trim();
            }
            ...
            request.responseTrace += "AANULL" + "\t" + categoryPath + "\n";
