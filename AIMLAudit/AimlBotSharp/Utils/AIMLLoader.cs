    ...
    public class AIMLLoader
    {
        ...
        public void loadAIML()
        {
            ...
            else
            {
                this.loadAIML(this.bot.PathToAIML+"\\pre");
                this.loadAIML(this.bot.PathToAIML);
                this.loadAIML(this.bot.PathToAIML + "\\post");
                this.loadAIML(this.bot.PathToAIML + "\\test");
                //this.loadAIML(this.bot.PathToAIML + "\\holding");
            }
        }
        ...
        public void loadAIMLFile(string filename)
        {
            ...
            // load the document
            Stream input0 = SMLTranslator.openSMLStream(filename);
            string strTxt = SMLTranslator.getSMLFile(filename);
        ...
        private void processCategory(XmlNode node, string topicName, string filename)
        {
            ...
            if (object.Equals(null, template))
            {
                ...
                // Professor / Chomsky assume "null template" == "erase"
            ...
            if (categoryPath.Length > 0)
            {
                try
                {
                    ...
                    if (template != null)
                    {
                        //templateText = template.OuterXml;
                        template.Attributes.RemoveNamedItem("xmlns");
                        string xo = template.InnerXml;
                        if (!xo.Contains("<") && !xo.Contains("&"))
                        {
                            xo = template.InnerText;
                        }
                        if (xo.Contains("index"))
                        {
                           // xo = xo;
                        }
                        templateText = "<template>" + xo + "</template>";
                    }
                    this.bot.Graphmaster.addCategory(categoryPath, templateText, filename,"");
                    this.bot.pathToFilename[categoryPath] = filename;
