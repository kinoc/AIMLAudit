...
namespace AIMLbot.Utils 
{
    ...
    { 
        ...
        protected bool TemplateNodeHasText(AIMLTagHandler tagHandler,Bot bot,XmlNode node)
        {
             //return ((string)Recurse(tagHandler,bot,node)).Length > 0;
            if (String.IsNullOrEmpty(node.InnerXml)) return false;
            if (String.IsNullOrEmpty(node.InnerXml.Trim())) return false;
            return true;
        }
        private AIMLTagHandler RecurseVistor = null;
        private AIMLTagHandler RecurseVistor2 = null;

        public string Recurse(AIMLTagHandler tagHandler,Bot bot,XmlNode node)
        {
            string resultNodeInnerXML = node.InnerXml;// tagHandler.Transform();
            ...
            if (resultNode.HasChildNodes)
            {
                ...
                {
                    recursiveResult.Append(bot.processNode(childNode, query, request, result, user));
                }
        ...
        public string sraiCall(string callText)
        {
            Request subRequest = new Request(callText, this.user, this.bot);
