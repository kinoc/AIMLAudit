            ...
            if (this.templateNode.Name.ToLower() == "srai")
            {
                if (this.templateNode.InnerText.Length > 0)
                {
                    ...
                    if (request.StartedOn.AddMilliseconds(request.bot.TimeOut) < DateTime.Now)
                    {
                        ...
                        this.request.responseTrace += "MAXTIMEOUT\n";
     
                        return string.Empty;
                    }
                    if (request.depth > 32)
                    {
                        request.bot.writeToLog("WARNING! Recursion Depth Exceeded. User: " + request.user.UserID + " raw input: \"" + request.rawInput + "\"");
                        request.hasTimedOut = true;
                        this.request.responseTrace += "MAXRECURSION\n";
                        return string.Empty;
                    }
                    ...
                    subRequest.skipUnderscores = this.request.skipUnderscores;
                    
                    Result subQuery = this.bot.Chat(subRequest);
                    this.request.hasTimedOut = subRequest.hasTimedOut;
                    this.request.responseTrace += subRequest.responseTrace;
